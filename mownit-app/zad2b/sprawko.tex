...
Już w tabeli \ref{tab:max_full_runge} można było zobaczyć, że dla $n=30$ błąd maksymalny zaczyna rosnąć. Dla większej liczby
węzłów interpolacji metoda jest jeszcze mniej stabilna. Jak moża zaobserwować w tabeli \ref{tab:high_n_comparison} błędy
numeryczne kaskadują znacznie szybciej dla metody Hermite'a niż dla metody Lagrange'a, niezależnie od tego czy porównujemy
metody dla takiej samej liczby węzłow, czy dla takiego samego stopnia wielomianu.
...
\section{Wnioski}
W zastosowaniach praktycznych metoda Hermite'a będzie zwykle lepsza jeżeli jesteśmy w stanie obliczyć pochodną funkcji interpolowanej z dużą dokładnością. 
Błędy numeryczne nie są znaczne dla stopni wielomianów poniżej 30 w żadnej z metod. Metoda Lagrange'a z z kolei będzie lepsza jeżeli funkcja
nie jest różniczkowalna, lub obliczenie wartości pochodnej byłoby obarczone błędem pomiarowym lub numerycznym.
\end{document}